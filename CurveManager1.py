...
class CurveManager1:
    def __init__(self, file_list=None):
        ...
        self.file_names = []
        # 读取配置文件
        with open('settings/curve_settings1.json', 'r', encoding='UTF-8') as settings_file:
    ...
    def process_all_csv_files(self):
        for one_data in self.uni_data:
            ...
            self.file_list = os.listdir(os.path.join(os.getcwd(), self.data_folder))
            print(self.file_list)
            if self.is_gas:
                self.file_list.sort(key=lambda x:x.split('-')[1])
            else:
    ...
    def draw_main_figure(self):
        ...
        if self.file_count == 1:
            ...
            if self.is_gas:
                mass_g = [item for item in self.lengend_list]
            else:
        ...
        # ax.tick_params(axis='both', direction=self.tick_param['direction'],) # pad标尺距轴线的距离
        # plt.minorticks_on()
        # 设置y轴的次刻度
        ...
        ax.grid(visible=self.grid["visible"])
